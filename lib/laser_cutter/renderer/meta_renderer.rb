# encoding: utf-8
require 'laser_cutter/helpers/colors'

class LaserCutter::Renderer::MetaRenderer < LaserCutter::Renderer::BaseRenderer

  META_KEYS = %w(units width height depth thickness notch kerf stroke padding margin page_size page_layout)

  def initialize(config = {})
    self.config    = config
    self.enclosure = LaserCutter::Geometry::Rect.create(LaserCutter::Geometry::Point[1, 1], 140, 150)
  end

  def render(pdf = nil)
    banner = <<-EOF
    Made with love and the Laser-Cutter (v#{LaserCutter::VERSION})
