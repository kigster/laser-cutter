require 'spec_helper'
require 'laser_cutter/strategy/line_joiner'
module LaserCutter
  module Model
    describe Box do
      let(:depth) { 70 }
      ...
      let(:config) { LaserCutter::Config(hash) }

      let(:box1_config) { LaserCutter::Config(box1_hash) }
      let(:box2_config) { LaserCutter::Config(box2_hash) }
      ...
      context '#notches' do
        ...
        context 'when notches are generated for a face' do
          subject { LaserCutter::Strategy::LineJoiner.new(box1.notches[:top]).lines }
