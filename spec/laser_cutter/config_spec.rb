...
RSpec.describe LaserCutter::Config do
  let(:config) { LaserCutter::Config.new(opts.merge(file: '/tmp/box.pdf')) }
  ...
  context '#validate' do
    context 'missing options' do
      ...
      it 'should be able to validate missing options' do
        ...
        expect { config.validate! }.to raise_error(LaserCutter::Model::MissingOption)
      end
    end
    context 'zero options' do
      ...
      it 'should be able to validate missing options' do
        ...
        expect { config.validate! }.to raise_error(LaserCutter::Model::ZeroValueNotAllowed)
      end
    end
  end

  context 'default values' do
    let(:opts) { { size: '2.0x2.0x2/0.125', file: '/tmp/a' } }
    it 'should correctly default notch based on thickness' do
      config.validate!
      expect(config.thickness).to eql(0.125)
      expect(config.notch).to eql(2.0 / 5)
    end
  end

  context 'with notch provided' do
    let(:opts) { { size: '2.0x2.0x2/0.125/0.2', file: '/tmp/a' } }
    it 'should correctly default notch based on thickness' do
      config.validate!
      expect(config.thickness).to eql(0.125)
      expect(config.notch).to eql(0.2)
    end
  end
