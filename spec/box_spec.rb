...
module Laser
  module Cutter
    describe Box do
      ...
      let(:config) { Laser::Cutter::Configuration.new(hash) }

      let(:box1_config) { Laser::Cutter::Configuration.new(box1_hash) }
      let(:box2_config) { Laser::Cutter::Configuration.new(box2_hash) }

      let(:box1) { Box.new(box1_config) }
      let(:box2) { Box.new(box2_config) }
